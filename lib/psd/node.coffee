...
module.exports = class Node extends Module
  ...
  @PROPERTIES: ['name', 'left', 'right', 'top', 'bottom', 'height', 'width',
    'imgSrc', 'textAlign', 'color', 'backgroundColor', 'fontWeight',
    'fontSize', 'verticalAlign', 'transform', 'delay', 'duration', 'lineHeight',
    'zindex', 'display', 'bg', 'iconKey','fontFamily', 'border', 'textIndent',
    'letterSpacing', 'audioSrc', 'playing', 'loop', 'des', 'locked', 'textLabelValue']
...
#    if @layer? then console.log(@layer) else console.log(2)
    @layer.node = @
    @_children = []
    ...
    @bg = @layer.bg
    @iconKey = @layer.iconKey
    @fontFamily = @layer.fontFamily
    @border = @layer.border
    @textIndent = @layer.textIndent
    @letterSpacing = @layer.letterSpacing
    @audioSrc = @layer.audioSrc
    @playing = @layer.playing
    @loop = @layer.loop
    @des = @layer.des
    @locked = @layer.locked
    @textLabelValue = @layer.textLabelValue
